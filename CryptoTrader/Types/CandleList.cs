...
namespace CryptoTrader.Types
{
    ...
    [Serializable]
    public class CandleList<T> : IList<T>, IList
    {
        ...
        //
        public CandleList(int capacity)
        {
            if (capacity < 0)
        ...
        // given collection.
        //
        public CandleList(IEnumerable<T> collection)
        {
            if (collection == null)
            ...
            get
            {
                lock (this)
                {
                    if (_maxSize.HasValue)
                        return _size > _maxSize.Value ? _maxSize.Value : _size;
                    else
                        return _size;
                }
            }
        ...
        {
            private readonly CandleList<T> _list;
            private int _index;
            private readonly int _version;
            private T? _current;

            internal Enumerator(CandleList<T> list)
            {
                _list = list;
                _index = 0;
